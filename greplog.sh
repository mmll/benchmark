...
#
svc=$1
echo "collect log for service: $svc"
set -euo pipefail
...
IFS=$'\n'
# Filter for ${svc} only spaces (could be enhanced if other spaces need inclusion e.g. Nautilus)
for i in $(cat $output_dir/kubectl-get-po-svc-pvc-all-namespace_output | grep -i "\-${svc}\-")
do
echo $i
# Detect ${svc} pods information in various K8 deployment (e.g. PKS / GKE)
if [[ $i == *"po/"* ]] || [[ $i == *"pod/"* ]]; then
...
# Detect if pod had been restarted and collect previous instance of the pod log if it exists
        ...
        #     kubectl -n `echo $i | awk '{print $1}'` logs `echo $i | awk '{print $2}'` -p -c $svc > $output_dir/log_`echo $i | awk 'gsub(/\//,"_") {print $2}'`_previous.log
        # fi
# Collect recent logs from pod
        kubectl -n `echo $i | awk '{print $1}'` logs `echo $i | awk '{print $2}'` -c $svc > $output_dir/log_`echo $i | awk 'gsub(/\//,"_") {print $2}'`_recent.log
# Detect if the pod have older logs due to logroate and collect all logs from that pod
        log_count=$(kubectl -n `echo $i | awk '{print $1}'` exec -it `echo $i |  awk '-F'[/] '{print $2}'|awk '{print $1}'` -c $svc -- ls -l /opt/storageos/logs 2>/dev/null | wc -l || true)
        if [ $log_count -ge 2 ]; then
            mkdir -p $output_dir/oldlogs_`echo $i |  awk '-F'[/] '{print $2}'|awk '{print $1}'`
            kubectl -n `echo $i | awk '{print $1}'` cp `echo $i |  awk '-F'[/] '{print $2}'|awk '{print $1}'`:/opt/storageos/logs $output_dir/oldlogs_`echo $i |  awk '-F'[/] '{print $2}'|awk '{print $1}'`/. -c $svc
        fi
    }
    else
...
cwd=`pwd`
mkdir -p $cwd/${svc}-k8s-logs
cd $output_dir
# Create compressed log archive with date stamp
tar zcf $cwd/${svc}-k8s-logs/${svc}-k8s-logging-`date '+%Y-%m-%d-%H-%M-%S'`.tgz *
rm -rf $output_dir
echo "Final logs are avaialble in `ls $cwd/${svc}-k8s-logs/*.tgz`"
tar -xvf `ls $cwd/${svc}-k8s-logs/*.tgz| tail -1` -C $cwd/${svc}-k8s-logs/
